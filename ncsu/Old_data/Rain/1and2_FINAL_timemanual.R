...
process_rain_data (file_path = "C:/Users/Lenovo/Downloads/SampleFiles3")
#EM061523.csv
#Second function to run
automatic_rain<-quick_check_plot (file_path = "C:/Users/Lenovo/Downloads/SampleFiles3")
OTIP_EM061523.csvvOTIP_EM061523.csv  #OTIP_EM061523.csv
#precipitacionescorona2023fomat.xlsx


#save the data frame with rainfall data to read by flumes
write.csv(automatic_rain$combined_data_frame_av2,file.path(output_dir, "rain_data"),sep = '/t')

#list of rainfall plots 
plot_list<-automatic_rain$plot_list
...
#  browser()
  ...
  #browser()
    combined_data_frame_av <-  combined_data_frame_av%>% 
    filter(!is.na(Fecha)) 
 #   mutate(Fecha2 = mdy_hms(as.Date(Fecha), tz = "America/Argentina/Buenos_Aires"))
 #   browser()
    
    #NEXT STEP WOULD BE EXTRACTING TIME FROM THE PLANILLA CUENCA
    combined_data_frame_av2<-combined_data_frame_av%>%
    mutate(date = as.Date(Fecha) + hours(10))%>%
    select(-Fecha)
    
#    combined_data_frame_av2<- na.omit(combined_data_frame_av2)
#    browser()
...
#  browser()
  ...
  combined_data_frame_plot<-full_join(result_df_plotmanual, combined_data_frame_av2, by=c("date"),all= TRUE)
 ...
 # browser()
  ...
  plot_list <- append(plot_list, list(pM))
  #browser()
  ...
  ggsave(quickplot_rain_manual, filename = "quickcheck3.png", width = 8, height = 6, dpi = 300)
   ...
   plot_html_filename <- file.path(output_dir, "rain_plots.html")
   htmltools::save_html(plot_list, plot_html_filename)
   
}  
output_list <- list(combined_data_frame_av2 = combined_data_frame_av2, plot_list = plot_list)
return(output_list)
}
