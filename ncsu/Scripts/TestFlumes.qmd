...
```{r read-files}
File_info <- read_list_flume("..//All_Raw_Data/T042223/flumes")
str(File_info)
```

Lets' also test the manual read of S1042223.csv (don't use quotes)

Testing the HOBOU20 function

```{r info-hobo}
test_info <- read_flume_files(file_path = "..//All_Raw_Data/T042223/flumes")
test_info
```
...
test_data <- file_process(test_info$names[1], test_info$paths[1],
                          test_info$instrument[1], output_dir = "../All_Raw_Data/T042223/flumes/processed")
ggplotly(test_data)
```
