...
```{r}
rain <- read_otip_file(name = "OTIP_r1052523.csv", 
                       path_to_file = paste0(Data_dir,"/rain/processed"))

flume <- read_flume_output(name = "S1052523_processed.csv",
...
p <- merged_df %>%
  ...
  geom_line(aes(`Date and Time`, `Water Level, meters`*10), colour = "red") + theme_bw() + 
  scale_y_continuous(sec.axis =sec_axis( trans=~./10, name="Water Level in Meters")) 
p
ggplotly(p)
